...
async function sendMessage(type, ip, message) {
    if (BotToken !== '' && ChatID !== '') {
        let msg = "";
...
async function recordDeviceInfo(ip, userAgent, hostname, url) {
    const message = `
        UA: ${userAgent}
        域名: ${hostname}
    ...
    await sendMessage("#获取订阅", ip, message);
}
...
export default {
    async fetch(request, env) {
        ...
        if (!(token == mytoken || url.pathname == ("/" + mytoken) || url.pathname.includes("/" + mytoken + "?"))) {
            ...
            await recordDeviceInfo(request.headers.get('CF-Connecting-IP'), userAgent, url.hostname, url);
        }
            ...
            try {
                ...
                return new Response(subconverterContent, {
                    headers: {
                        "Content-Disposition": `attachment; filename*=utf-8''${encodeURIComponent(FileName)}; filename=${FileName}`,
                        "content-type": "text/plain; charset=utf-8",
                        "Profile-Update-Interval": `${SUBUpdateTime}`,"Subscription-Userinfo": `upload=${UD}; download=${UD}; total=${total}; expire=${expire}`,
                    },
                });
            } catch (error) {
                return new Response(`Error: ${error.message}`, {
